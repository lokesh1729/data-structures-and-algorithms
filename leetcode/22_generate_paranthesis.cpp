...
public:
    vector<string> generateParenthesis(int n)
    {
        ...
        generateParenthesis_backtracking(res, "", 0, 0, n);
        // bfs(res, n);
        return res;
    }
    void generateParenthesis_backtracking(vector<string> &res, string curr, int open, int close, int max)
    {
        if (curr.size() == max * 2)
        {
            res.push_back(curr);
        ...
        if (open < max)
        {
            generateParenthesis_backtracking(res, curr + '(', open + 1, close, max);
        }
        if (close < open)
        {
            generateParenthesis_backtracking(res, curr + ')', open, close + 1, max);
        }
    }
